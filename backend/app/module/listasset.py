...
class ListAsset(Resource):
    ...
    def get(self):
        ...
        lmd.execute("SELECT * FROM assets where status != %s", ('Deleted',))
        data = lmd.fetchall()
        lmd.close()
        formatted_data = [{
            'row': index + 1,
            ...
            'image_path': item[10],
            'qrcode_path': item[11],
            'created_at': item[12].strftime("%Y-%m-%d %H:%M:%S"),
...
class DeleteAsset(Resource):
    ...
    def delete(self, asset_id):
        ...
        if existing_asset:
            ...
            lmd.execute("UPDATE assets SET status = %s, photo = %s, qrcode = %s WHERE id = %s", ('Deleted', image_path, image_path, asset_id))
...
class ListAssetExcept(Resource):
    ...
    def get(self):
        db, lmd = get_db_connection()
        lmd.execute("SELECT * FROM assets WHERE status  IN ('Available')")
        data = lmd.fetchall()
        lmd.close()
        formatted_data = [{
            'no': item[0],
